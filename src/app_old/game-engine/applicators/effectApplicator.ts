import type { Mechanic, GameContext, ModifiedStats, ModifiedModelStats, ModifiedWeaponStats, RollModifier, RerollType, MechanicSource } from "../types";
 ...
 */
function createInitialStats(context: GameContext, perspective: "attacker" | "defender"): ModifiedStats {
...
function applyStaticNumber(stats: ModifiedStats, mechanic: Mechanic): void {
    ...
    // Model attributes
    const modelAttrs: (keyof ModifiedModelStats)[] = ["m", "t", "sv", "invSv", "w", "ld", "oc"];
    ...
    if (stats.weapon && mechanic.attribute) {
        const weaponAttrs: (keyof ModifiedWeaponStats)[] = ["range", "a", "bsWs", "s", "ap", "d"];
...
function applyReroll(stats: ModifiedStats, mechanic: Mechanic): void {
    const rerollType: RerollType = mechanic.value === "ones" ? "ones" : mechanic.value === "failed" ? "failed" : mechanic.value === "all" ? "all" : "all"; // Default to all if just "true"

    switch (mechanic.attribute) {
        case "h":
            // Upgrade reroll (ones < failed < all)
            if (stats.rerollHits === "none" || (stats.rerollHits === "ones" && rerollType !== "ones") || (stats.rerollHits === "failed" && rerollType === "all")) {
                stats.rerollHits = rerollType;
            }
            break;
        case "w":
            if (stats.rerollWounds === "none" || (stats.rerollWounds === "ones" && rerollType !== "ones") || (stats.rerollWounds === "failed" && rerollType === "all")) {
                stats.rerollWounds = rerollType;
            }
            break;
        case "s":
            if (stats.rerollSaves === "none" || (stats.rerollSaves === "ones" && rerollType !== "ones") || (stats.rerollSaves === "failed" && rerollType === "all")) {
...
/**
 * Applies a criticalWoundThreshold mechanic (ANTI-X abilities).
 * Sets the threshold at which wound rolls become critical wounds.
 * Uses the lowest (best) threshold if multiple apply.
 */
function applyCriticalWoundThreshold(stats: ModifiedStats, mechanic: Mechanic): void {
    const threshold = typeof mechanic.value === "number" ? mechanic.value : null;
    if (threshold !== null) {
        // Use the best (lowest) threshold
        if (stats.antiThreshold === null || threshold < stats.antiThreshold) {
            stats.antiThreshold = threshold;
        }
    }
}
...
function applyMechanic(stats: ModifiedStats, mechanic: Mechanic): void {
    switch (mechanic.effect) {
        ...
        case "reroll":
            ...
            break;
        case "criticalWoundThreshold":
            applyCriticalWoundThreshold(stats, mechanic);
            break;
 ...
 */
export function applyEffects(mechanics: Mechanic[], context: GameContext, perspective: "attacker" | "defender"): ModifiedStats {
    const stats = createInitialStats(context, perspective);

    // Filter to only applicable mechanics
    const applicableMechanics = filterApplicableMechanics(mechanics, context, perspective);
    ...
    const sortedMechanics = [...applicableMechanics].sort((a, b) => (effectOrder[a.effect] ?? 99) - (effectOrder[b.effect] ?? 99));
...
export function applyEffectsWithDetails(
    ...
    const sortedApplied = [...applied].sort((a, b) => (effectOrder[a.effect] ?? 99) - (effectOrder[b.effect] ?? 99));
