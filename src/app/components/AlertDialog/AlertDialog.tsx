    ...
    return (
        <AlertDialogPortal>
            ...
            <AlertDialogPrimitive.Content
                ...
                className={cn(
                    "bg-wordBearersRed text-wildRiderRed data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded border-wildRiderRed border-1 p-6 shadow-lg duration-200 sm:max-w-lg",
                    className
                )}
                {...props}
            />
...
function AlertDialogCancel({ className, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Cancel>) {
    return <AlertDialogPrimitive.Cancel className={cn(buttonVariants({ variant: "ghostDestructive" }), className)} {...props} />;
}
