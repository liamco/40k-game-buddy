...
import { buttonVariants } from "#components/Button/Button.tsx";

type AlertDialogVariant = "default" | "warning" | "destructive";

const AlertDialogContext = React.createContext<AlertDialogVariant>("default");

function AlertDialog({ variant = "default", ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Root> & { variant?: AlertDialogVariant }) {
    return (
...
const alertDialogCancelVariants = {
    default: "ghost",
    warning: "ghostWarning",
    destructive: "ghostDestructive",
} as const;

function AlertDialogAction({ className, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Action>) {
    const variant = React.useContext(AlertDialogContext);
    return <AlertDialogPrimitive.Action className={cn(buttonVariants({ variant: alertDialogActionVariants[variant] }), className)} {...props} />;
}

function AlertDialogCancel({ className, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Cancel>) {
    const variant = React.useContext(AlertDialogContext);
    return <AlertDialogPrimitive.Cancel className={cn(buttonVariants({ variant: alertDialogCancelVariants[variant] }), className)} {...props} />;
}
