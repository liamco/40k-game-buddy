...
import type { Faction, FactionIndex, ArmyList, ArmyListItem, Datasheet, Detachment, LoadoutConstraint } from "../../types";
...
export function ListManager() {
    ...
    useEffect(() => {
        const loadBodyguardUnits = async () => {
            ...
            for (const datasheetRef of selectedItem.leaders) {
                try {
                    const fullDatasheet = await loadDatasheetData(selectedItem.factionSlug, datasheetRef.id);
    ...
    const addDatasheetToList = (datasheet: Datasheet) => {
        ...
        loadDatasheetData(selectedList.factionSlug, datasheet.id).then((fullDatasheet) => {
            ...
            const duplicateItems = selectedList.items.filter((item) => getBaseName(item.name) === baseName);
    ...
    const attachLeaderToUnit = (leaderItemId: string, targetUnitItemId: string) => {
        ...
        const previousTargetItem = leaderItem.leading ? selectedList.items.find((item) => item.id === leaderItem.leading?.id && item.name === leaderItem.leading?.name) : null;
    ...
    const detachLeaderFromUnit = (leaderItemId: string) => {
        ...
        const targetUnitItem = selectedList.items.find((item) => item.id === leaderItem.leading?.id && item.name === leaderItem.leading?.name);
    ...
    const attachEnhancementToLeader = (leaderItemId: string, enhancement: { id: string; name: string; cost?: number }) => {
        if (!selectedList) return;
    ...
    const updateLoadoutSelection = (itemId: string, optionLine: number, newCount: number, maxAllowed: number) => {
        if (!selectedList) return;
    ...
    return (
        <div className="min-h-screen p-6">
            <div className="mb-6">
                ...
                <p className="">Create and manage your army lists</p>
            </div>
                    ...
                    <Card>
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                ...
                                <Button size="sm" onClick={() => setIsCreatingList(true)} className="h-8">
                            ...
                            {isCreatingList ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-2">
                                        <Button onClick={createNewList} disabled={!selectedFaction || !selectedDetachment || !newListName.trim()} className="flex-1">
                                            Create List
                                        </Button>
                                        <Button
                                            variant="outline"
                                ...
                                <div className="space-y-2">
                                    {lists.length === 0 ? (
                                        ...
                                        lists.map((list) => (
                                            <div
                                                key={list.id}
                                                className={`p-3 rounded-lg border cursor-pointer transition-colors ${selectedList?.id === list.id ? "bg-blue-50 border-blue-300" : "bg-white border-[#e6e6e6] hover:border-blue-200"}`}
                                                onClick={() => {
                                            ...
                                            >
                                                <div className="flex items-start justify-between">
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-xs ">{list.items.length} units</p>
                                                    </div>
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        {selectedList.items.length === 0 ? (
                                            <p className="text-sm  text-center py-8 border border-dashed rounded-lg mt-2">No units added yet. Use the search above to add units.</p>
                                        ) : (
                        ...
                        <Card>
                            <CardContent className="flex items-center justify-center h-64">
                                <div className="text-center">
                                    <p className=" mb-2">Select a list from the left or create a new one</p>
                                </div>
                            </CardContent>
                        </Card>
                    )}
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                {selectedItem && (
                                                                    <Button
                                                                        ...
                                                                        onClick={() => {
                                                                            attachEnhancementToLeader(selectedItem.listItemId, {
                                                                                id: enhancement.id,
                                                                                name: enhancement.name,
                                                                                cost: enhancement.cost,
                                                                            });
