import type { GameContext, CombatResult, AppliedMechanic, ModifiedStats } from "../types";
...
import { calculateSaveTarget, getFailedSaveProbability } from "./saveCalculator";
...
export function calculateCombat(context: GameContext): CombatResult {
    ...
    const { stats: attackerStats, applied: attackerApplied, notApplied: attackerNotApplied } = applyEffectsWithDetails(attackerMechanics, context, "attacker");

    // Apply defender mechanics
    const { stats: defenderStats, applied: defenderApplied, notApplied: defenderNotApplied } = applyEffectsWithDetails(defenderMechanics, context, "defender");
    ...
    const hitResult = calculateHitTarget(bsWs, attackerStats.rollModifiers.hit, attackerStats.autoHit);

    // Get weapon strength and defender toughness
    const strength = attackerStats.weapon?.s ?? 4;
    const toughness = defenderStats.model.t;

    // Calculate wound roll (including ANTI-X threshold if applicable)
    const woundResult = calculateWoundTarget(strength, toughness, attackerStats.rollModifiers.wound, attackerStats.autoWound, attackerStats.antiThreshold);

    // Check for IGNORES COVER
    const ignoresCover = attackerStats.addedKeywords.includes("IGNORES COVER") || attackerStats.weapon?.attributes?.includes("IGNORES COVER");

    // Calculate save roll
    const saveResult = calculateSaveTarget(defenderStats.model.sv, defenderStats.model.invSv, attackerStats.weapon?.ap ?? 0, context.defender.state.inCover, ignoresCover ?? false, defenderStats.rollModifiers.save);
    ...
    const damageResult = calculateExpectedDamage(attacks, getHitProbability(hitResult.target, hitResult.autoHit), getWoundProbability(woundResult.target, woundResult.autoWound), getFailedSaveProbability(saveResult.target), damage, fnp);
 ...
 */
export function getModifiedStats(context: GameContext, perspective: "attacker" | "defender"): ModifiedStats {
    const { attackerMechanics, defenderMechanics } = collectAllMechanics(context);
    const mechanics = perspective === "attacker" ? attackerMechanics : defenderMechanics;
