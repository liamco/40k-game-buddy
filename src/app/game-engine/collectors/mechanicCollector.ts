...
function collectAttackerMechanics(context: GameContext): Mechanic[] {
  ...
  // 2. Weapon attributes
  const weaponMechanics = collectWeaponAttributes(
    context.attacker.selectedWeapon
  );
  ...
  const detachmentMechanics = collectDetachmentAbilities(
  ...
  // 5. Active stratagems
  const stratagemMechanics = collectStratagems(
    context.attackerStratagems,
    context
  );
